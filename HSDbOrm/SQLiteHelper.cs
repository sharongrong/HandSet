...
namespace HSDbOrm
{
    public class SQLiteHelper
    {
        ...
        /// 构造函数传入数据库
        /// </summary>
        public SQLiteHelper(string dbName, string password)
        {
            _dbName = dbName;
            _password = password;
        }
        ...
        {//"Data Source=Test.db3;Pooling=true;FailIfMissing=false";
            get
            {
                ...
                return "Data Source=" + _dbName + ";Password=" + _password + ";Pooling=true;FailIfMissing=false";
            }
        }
        ...
        public void UpdateDataset(SQLiteCommand selectCommand, SQLiteCommand insertCommand, SQLiteCommand deleteCommand, SQLiteCommand updateCommand, DataSet dataSet, string tableName)
        {
        ...
        /// 执行多条命令
        /// </summary>
        /// <param name="commandText">多条命令</param>
        /// <returns></returns>
        public int ExecuteMutliQuery(string[] commandText)
        {
            int res = 0;
            if (Conn.State == ConnectionState.Closed)
                Conn.Open();
            using (SQLiteTransaction dbTrans = Conn.BeginTransaction())
            {
                try
                {
                    foreach (string row in commandText)
                    {
                        res += ExecuteNonQuery(dbTrans, row, null);
                    }
                    dbTrans.Commit();
                }
                catch
        ...
        /// <param name="dtData"></param>
        /// <returns></returns>
        public int ExecuteMutliQuery(string commandText, DataTable dtData, out string mess)
        {
            int res = 0;
            mess = "";
            if (Conn.State == ConnectionState.Closed)
        ...
        public int ExecuteMutliQuery(string commandText, string[][] dtData)
        {
            ...
            using (SQLiteTransaction dbTrans = Conn.BeginTransaction())
            {
                try
                {
                    foreach (string[] row in dtData)
                    {
                        res += ExecuteNonQuery(dbTrans, commandText, row);
                    }
                    dbTrans.Commit();
                }
                catch
